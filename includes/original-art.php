...
add_action('woocommerce_process_shop_order_meta', function($order_id): void {
  sunstreaker_generate_original_art_for_order($order_id);
  sunstreaker_generate_mockup_assets_for_order($order_id);
}, 50, 1);
...
function sunstreaker_sync_generated_original_art_meta($item, string $png_url, string $eps_url = '', string $slot = 'back'): void {
  if (!$item || !is_a($item, 'WC_Order_Item_Product')) return;
  ...
  $eps_url = trim($eps_url);
  $slot = strtolower(trim($slot)) === 'front' ? 'front' : 'back';
  $label = $slot === 'front' ? 'Front' : 'Back';
  $png_key = 'Original Art — '.$label.' PNG';
  $eps_key = 'Original Art — '.$label.' EPS';
  $prefer_eps = sunstreaker_order_item_original_art_prefers_eps($item, $slot);

  if (!$prefer_eps && $png_url !== '') {
    $item->update_meta_data($png_key, esc_url_raw($png_url));
  } else {
    $item->delete_meta_data($png_key);
  }

  if ($prefer_eps && $eps_url !== '') {
    $item->update_meta_data($eps_key, esc_url_raw($eps_url));
  } else {
...
function sunstreaker_order_item_original_art_prefers_eps($item, string $slot): bool {
  $slot = strtolower(trim($slot)) === 'front' ? 'front' : 'back';
  $method = '';

  $product = $item && is_a($item, 'WC_Order_Item_Product') ? $item->get_product() : null;
  $product_id = $product && is_a($product, 'WC_Product') ? (int) $product->get_id() : ($item ? (int) $item->get_product_id() : 0);
  if ($product_id > 0 && function_exists('sunstreaker_get_feature_production_method')) {
    $method = sunstreaker_get_feature_production_method($product_id, $slot === 'front' ? 'front_back' : 'name_number');
  }

  if (trim((string) $method) === '') {
    $method = $item ? (string) $item->get_meta('Production', true) : '';
  }

  return stripos((string) $method, 'screen print') !== false;
}

function sunstreaker_preferred_mockup_display_url(string $svg_url, string $webp_url = '', string $png_url = ''): string {
  ...
  $png_url = trim($png_url);
  if ($png_url !== '') {
    return esc_url_raw($png_url);
  }

  $svg_url = trim($svg_url);
  return $svg_url !== '' ? esc_url_raw($svg_url) : '';
}
...
function sunstreaker_sync_generated_mockup_meta($item, string $mockup_url): void {
  ...
  if ($mockup_url !== '') {
    $item->update_meta_data('Mockup', esc_url_raw($mockup_url));
    return;
  }
...
function sunstreaker_ensure_cart_item_mockup_data(array $cart_item_data, int $product_id = 0, int $variation_id = 0): array {
  ...
  $existing_png_url = trim((string) ($ss['mockup_png_url'] ?? ''));
  if ($existing_png_url !== '' && filter_var($existing_png_url, FILTER_VALIDATE_URL)) {
    $cart_item_data['sunstreaker']['mockup_png_url'] = esc_url_raw($existing_png_url);
    $cart_item_data['sunstreaker']['mockup_url'] = esc_url_raw($existing_png_url);
    return $cart_item_data;
  }
	  ...
	  if ($existing_svg_url !== '' && filter_var($existing_svg_url, FILTER_VALIDATE_URL)) {
	    ...
	    $cart_item_data['sunstreaker']['mockup_url'] = sunstreaker_preferred_mockup_display_url($existing_svg_url, (string) ($ss['mockup_webp_url'] ?? ''), (string) ($ss['mockup_png_url'] ?? ''));
	    return $cart_item_data;
	  }
...
function sunstreaker_generate_original_art_for_order($order_id): void {
  ...
  foreach ($order->get_items('line_item') as $item_id => $item) {
    ...
    $existing_front_png_url = trim((string) $item->get_meta('_sunstreaker_front_original_art_png_url', true));
    $existing_front_eps_url = trim((string) $item->get_meta('_sunstreaker_front_original_art_eps_url', true));
    if ($item->get_meta('_sunstreaker_front_original_art_generated', true) === 'yes' && $existing_front_png_url !== '' && $existing_front_eps_url !== '') {
      sunstreaker_sync_generated_original_art_meta($item, $existing_front_png_url, $existing_front_eps_url, 'front');
    } else {
      $front_result = sunstreaker_generate_front_original_art_for_item($order, (int) $item_id, $item);
      if (is_wp_error($front_result)) {
        $item->update_meta_data('_sunstreaker_front_original_art_error', $front_result->get_error_message());
        $order->add_order_note(
          sprintf(
            'Sunstreaker: failed to generate Original Art Front for item %d (%s).',
            (int) $item_id,
            $front_result->get_error_message()
          )
        );
      } elseif (is_array($front_result) && !empty($front_result['generated'])) {
        $item->delete_meta_data('_sunstreaker_front_original_art_error');
        $item->update_meta_data('_sunstreaker_front_original_art_generated', 'yes');
        $item->update_meta_data('_sunstreaker_front_original_art_generated_at', current_time('mysql'));
        if (!empty($front_result['eps_url'])) $item->update_meta_data('_sunstreaker_front_original_art_eps_url', esc_url_raw($front_result['eps_url']));
        if (!empty($front_result['png_url'])) $item->update_meta_data('_sunstreaker_front_original_art_png_url', esc_url_raw($front_result['png_url']));
        sunstreaker_sync_generated_original_art_meta(
          $item,
          (string) ($front_result['png_url'] ?? ''),
          (string) ($front_result['eps_url'] ?? ''),
          'front'
        );
        $order->add_order_note(
          sprintf(
            'Sunstreaker: Original Art Front generated for item %d. EPS: %s PNG: %s',
            (int) $item_id,
            (string) ($front_result['eps_url'] ?? 'n/a'),
            (string) ($front_result['png_url'] ?? 'n/a')
          )
        );
      }
    }

    $browser_back_png_url = trim((string) $item->get_meta('_sunstreaker_original_art_back_browser_png_url', true));
    $original_art_version = $browser_back_png_url !== '' ? 'browser-canvas-v1' : 'mockup-text-v3';
    $existing_version = trim((string) $item->get_meta('_sunstreaker_original_art_version', true));
    $existing_png_url = trim((string) $item->get_meta('_sunstreaker_original_art_png_url', true));
    $existing_eps_url = trim((string) $item->get_meta('_sunstreaker_original_art_eps_url', true));
    if ($item->get_meta('_sunstreaker_original_art_generated', true) === 'yes' && $existing_version === $original_art_version && $existing_png_url !== '' && $existing_eps_url !== '') {
      sunstreaker_sync_generated_original_art_meta($item, $existing_png_url, $existing_eps_url, 'back');
      $item->save();
      continue;
    }

    if ($browser_back_png_url !== '') {
      $browser_result = sunstreaker_generate_original_art_for_item_from_browser_png($order, (int) $item_id, $item, $browser_back_png_url);
      if (is_wp_error($browser_result)) {
        $item->update_meta_data('_sunstreaker_original_art_error', $browser_result->get_error_message());
        $item->save();
        $order->add_order_note(
          sprintf(
            'Sunstreaker: failed to generate Original Art Back for item %d (%s).',
            (int) $item_id,
            $browser_result->get_error_message()
          )
        );
        continue;
      }
      ...
      $item->delete_meta_data('_sunstreaker_original_art_error');
      $item->update_meta_data('_sunstreaker_original_art_version', $original_art_version);
      $item->update_meta_data('_sunstreaker_original_art_generated_at', current_time('mysql'));
      if (!empty($browser_result['eps_url'])) $item->update_meta_data('_sunstreaker_original_art_eps_url', esc_url_raw($browser_result['eps_url']));
      if (!empty($browser_result['png_url'])) $item->update_meta_data('_sunstreaker_original_art_png_url', esc_url_raw($browser_result['png_url']));
      sunstreaker_sync_generated_original_art_meta(
        $item,
        (string) ($browser_result['png_url'] ?? ''),
        (string) ($browser_result['eps_url'] ?? ''),
        'back'
      );
      $item->save();
      $order->add_order_note(
        sprintf(
          'Sunstreaker: Original Art Back generated for item %d from browser canvas. EPS: %s PNG: %s',
          (int) $item_id,
          (string) ($browser_result['eps_url'] ?? 'n/a'),
          (string) ($browser_result['png_url'] ?? 'n/a')
        )
      );
      continue;
    }
    ...
    if (!empty($result['eps_url'])) $item->update_meta_data('_sunstreaker_original_art_eps_url', esc_url_raw($result['eps_url']));
    if (!empty($result['png_url'])) {
      $item->update_meta_data('_sunstreaker_original_art_png_url', esc_url_raw($result['png_url']));
      sunstreaker_sync_generated_original_art_meta($item, (string) $result['png_url'], (string) ($result['eps_url'] ?? ''), 'back');
    } else {
      $item->delete_meta_data('_sunstreaker_original_art_png_url');
      sunstreaker_sync_generated_original_art_meta($item, '', (string) ($result['eps_url'] ?? ''), 'back');
    }
    $item->save();

    $order->add_order_note(
      sprintf(
        'Sunstreaker: Original Art Back generated for item %d. EPS: %s PNG: %s',
        (int) $item_id,
        (string) ($result['eps_url'] ?? 'n/a'),
        (string) ($result['png_url'] ?? 'n/a')
      )
    );
  }
}
...
function sunstreaker_generate_original_art_for_item($order, int $item_id, $item, string $name, string $number) {
  ...
  $eps_result = sunstreaker_write_eps_from_image($paths['png_path'], $paths['eps_path']);
  if (is_wp_error($eps_result)) {
    return $eps_result;
  }

  $svg_url = sunstreaker_upload_url_from_path($paths['svg_path']);
  $eps_url = sunstreaker_upload_url_from_path($paths['eps_path']);
  $png_url = sunstreaker_upload_url_from_path($paths['png_path']);

  return [
    'svg_url' => $svg_url ? $svg_url : '',
    'eps_url' => $eps_url ? $eps_url : '',
...
function sunstreaker_generate_original_art_for_item_from_browser_png($order, int $item_id, $item, string $png_url) {
  $source_path = sunstreaker_local_path_from_url($png_url);
  if ($source_path === '' || !is_readable($source_path)) {
    return new WP_Error('sunstreaker_browser_original_art_missing', 'Browser-rendered back original art file could not be found.');
  }

  $paths = sunstreaker_prepare_original_art_paths((int) $order->get_id(), $item_id, 'back');
  if (is_wp_error($paths)) return $paths;

  $png_result = sunstreaker_convert_image_to_png($source_path, $paths['png_path']);
  if (is_wp_error($png_result)) return $png_result;

  $eps_result = sunstreaker_write_eps_from_image($paths['png_path'], $paths['eps_path']);
  if (is_wp_error($eps_result)) return $eps_result;
...
function sunstreaker_get_front_original_art_source_url($item): string {
  if (!$item || !is_a($item, 'WC_Order_Item_Product')) return '';

  $front_art_url = esc_url_raw((string) $item->get_meta('_sunstreaker_front_art_url', true));
  if ($front_art_url !== '') return $front_art_url;

  // Logo-only front prints do not have uploaded artwork. Use the selected
  // front logo as their production-art source so Soundwave receives the same
  // Original Art Front field as uploaded-art orders.
  $locations = maybe_unserialize($item->get_meta('_sunstreaker_logo_locations', true));
  ...
  if (is_array($locations)) {
    foreach ($locations as $location) {
      ...
      $key = strtolower(trim((string) ($location['location_key'] ?? '')));
      $label = strtolower(trim((string) ($location['location_label'] ?? '')));
      if ($key !== 'front' && $label !== 'front') continue;
      $logo_url = esc_url_raw((string) ($location['logo_url'] ?? ''));
...
function sunstreaker_generate_front_original_art_for_item($order, int $item_id, $item) {
  $front_art_url = sunstreaker_get_front_original_art_source_url($item);
  if ($front_art_url === '') {
    return ['generated' => false];
  }

  $source_path = sunstreaker_local_path_from_url($front_art_url);
  if ($source_path === '' || !is_readable($source_path)) {
    return new WP_Error('sunstreaker_front_original_art_missing', 'Front uploaded artwork file could not be found.');
  }

  $paths = sunstreaker_prepare_original_art_paths((int) $order->get_id(), $item_id, 'front');
  if (is_wp_error($paths)) return $paths;
...
function sunstreaker_prepare_original_art_paths(int $order_id, int $item_id, string $slot = 'back') {
  $uploads = wp_upload_dir(null, false);
  if (!empty($uploads['error'])) {
    return new WP_Error('sunstreaker_original_art_upload_path_error', (string) $uploads['error']);
  }

  $dir = trailingslashit($uploads['basedir']).'sunstreaker-order-art/'.gmdate('Y/m');
  if (!wp_mkdir_p($dir)) {
    return new WP_Error('sunstreaker_original_art_mkdir_failed', 'Could not create original art upload directory.');
  }

  $slot = strtolower(trim($slot)) === 'front' ? 'front' : 'back';
  $base = sanitize_file_name('original-art-'.$slot.'-order-'.$order_id.'-item-'.$item_id);
  $svg_filename = wp_unique_filename($dir, $base.'.svg');
  $svg_path = trailingslashit($dir).$svg_filename;
  $png_path = trailingslashit($dir).preg_replace('/\.svg$/i', '.png', $svg_filename);
  $eps_path = trailingslashit($dir).preg_replace('/\.svg$/i', '.eps', $svg_filename);
...
function sunstreaker_prepare_mockup_paths(int $order_id, int $item_id) {
  ...
  $svg_filename = wp_unique_filename($dir, $base.'.svg');
	  ...
	  $webp_path = trailingslashit($dir).preg_replace('/\.svg$/i', '.webp', $svg_filename);
...
function sunstreaker_convert_svg_to_png(string $svg_path, string $png_path, int $dpi = 0) {
  ...
  $convert_svg_path = sunstreaker_prepare_svg_for_imagemagick($svg_path, dirname($png_path));
  if (is_wp_error($convert_svg_path)) return $convert_svg_path;
  $command .= ' -background none -alpha set -define png:color-type=6 '.escapeshellarg($convert_svg_path).' '.escapeshellarg($png_path).' 2>&1';
...
function sunstreaker_prepare_svg_for_imagemagick(string $svg_path, string $asset_dir) {
  $svg = @file_get_contents($svg_path);
  if ($svg === false || strpos($svg, 'href=') === false) return $svg_path;

  if (!wp_mkdir_p($asset_dir)) {
    return new WP_Error('sunstreaker_svg_asset_dir_failed', 'Could not prepare SVG raster assets.');
  }

  $changed = false;
  $updated = preg_replace_callback('/\b(?:xlink:)?href="([^"]+)"/i', function(array $matches) use ($asset_dir, &$changed): string {
    $raw = html_entity_decode((string) ($matches[1] ?? ''), ENT_QUOTES | ENT_HTML5, 'UTF-8');
    $path = '';

    if (preg_match('/^data:([^;,]+);base64,(.+)$/i', $raw, $data_matches)) {
      $mime = strtolower(trim((string) $data_matches[1]));
      $bytes = base64_decode((string) $data_matches[2], true);
      if ($bytes === false || $bytes === '') return $matches[0];
      $ext = 'bin';
      if ($mime === 'image/png') $ext = 'png';
      elseif ($mime === 'image/jpeg' || $mime === 'image/jpg') $ext = 'jpg';
      elseif ($mime === 'image/webp') $ext = 'webp';
      elseif ($mime === 'image/svg+xml') $ext = 'svg';
      $path = trailingslashit($asset_dir).'svg-asset-'.md5($raw).'.'.$ext;
      if (!file_exists($path)) @file_put_contents($path, $bytes);
    } else {
      $path = sunstreaker_local_path_from_url($raw);
    }

    if ($path === '' || !is_readable($path)) return $matches[0];
    $changed = true;
    return str_replace($matches[1], esc_attr($path), $matches[0]);
  }, $svg);

  if (!$changed || !is_string($updated) || $updated === '') return $svg_path;

  $prepared_path = trailingslashit($asset_dir).'prepared-'.basename($svg_path);
  if (@file_put_contents($prepared_path, $updated) === false) {
    return new WP_Error('sunstreaker_svg_prepare_failed', 'Could not prepare SVG for raster conversion.');
  }

  return $prepared_path;
}

function sunstreaker_imagemagick_convert(string $source_path, string $target_path, array $args = []) {
  if (!file_exists($source_path)) {
    return new WP_Error('sunstreaker_convert_source_missing', 'Source file not found for conversion.');
  }
  ...
  if (!empty($args['density'])) {
    $command .= ' -units PixelsPerInch -density '.(int) $args['density'];
  }

  $command .= ' '.escapeshellarg($source_path);
  if (!empty($args['background_none'])) {
    $command .= ' -background none -alpha set';
  }
  if (!empty($args['flatten'])) {
    $command .= ' -background white -alpha remove -alpha off';
  }
  if (!empty($args['png'])) {
    $command .= ' -define png:color-type=6';
  }
  $command .= ' '.escapeshellarg($target_path).' 2>&1';

  $output = [];
  $code = 0;
  @exec($command, $output, $code);
  if ($code !== 0 || !file_exists($target_path)) {
    return new WP_Error(
      'sunstreaker_convert_failed',
      trim(implode("\n", array_filter(array_map('strval', $output)))) ?: 'Could not convert generated art.'
    );
  }
...
function sunstreaker_convert_svg_to_eps(string $svg_path, string $eps_path, int $dpi = 0) {
  return sunstreaker_imagemagick_convert($svg_path, $eps_path, [
    'density' => $dpi,
    'background_none' => true,
  ]);
}

function sunstreaker_convert_image_to_png(string $source_path, string $png_path) {
  return sunstreaker_imagemagick_convert($source_path, $png_path, [
    'background_none' => true,
    'png' => true,
  ]);
}

function sunstreaker_convert_image_to_eps(string $source_path, string $eps_path) {
  return sunstreaker_imagemagick_convert($source_path, $eps_path, [
    'flatten' => true,
  ]);
}

function sunstreaker_load_image_resource(string $path) {
  $type = '';
  if (!is_readable($path)) {
    return new WP_Error('sunstreaker_eps_source_missing', 'EPS source image could not be read.');
  }

  if (function_exists('wp_check_filetype')) {
    $type = strtolower((string) (wp_check_filetype($path)['type'] ?? ''));
  }
  if ($type === '' && function_exists('mime_content_type')) {
    $type = strtolower((string) @mime_content_type($path));
  }

  if ($type === 'image/png' && function_exists('imagecreatefrompng')) {
    $image = @imagecreatefrompng($path);
  } elseif (($type === 'image/jpeg' || $type === 'image/jpg') && function_exists('imagecreatefromjpeg')) {
    $image = @imagecreatefromjpeg($path);
  } elseif ($type === 'image/webp' && function_exists('imagecreatefromwebp')) {
  ...
  if (!$image) {
    return new WP_Error('sunstreaker_eps_source_unsupported', 'EPS source image type is not supported by GD.');
  }

  return $image;
}

function sunstreaker_write_eps_from_image(string $source_path, string $eps_path) {
  if (!function_exists('imagecolorat')) {
    return sunstreaker_write_eps_from_image_with_imagemagick_rgb($source_path, $eps_path);
  }

  $image = sunstreaker_load_image_resource($source_path);
  ...
  if ($width <= 0 || $height <= 0) {
    imagedestroy($image);
    return new WP_Error('sunstreaker_eps_empty_image', 'EPS source image has invalid dimensions.');
  }

  $fh = @fopen($eps_path, 'wb');
  if (!$fh) {
    imagedestroy($image);
    return new WP_Error('sunstreaker_eps_write_failed', 'Could not write EPS art file.');
  }

  sunstreaker_write_eps_vector_header($fh, $width, $height);
  ...
  for ($y = 0; $y < $height; $y++) {
    ...
    for ($x = 0; $x <= $width; $x++) {
      if ($x >= $width) {
        if (is_array($run)) {
          sunstreaker_write_eps_pixel_run($fh, $run, $height);
          $run = null;
        }
        break;
      }
  ...
  return file_exists($eps_path) ? $eps_path : new WP_Error('sunstreaker_eps_write_failed', 'Could not write EPS art file.');
}

function sunstreaker_write_eps_vector_header($handle, int $width, int $height): void {
  fwrite($handle, "%!PS-Adobe-3.0 EPSF-3.0\n");
  fwrite($handle, "%%Creator: Sunstreaker\n");
  fwrite($handle, "%%BoundingBox: 0 0 ".$width." ".$height."\n");
  fwrite($handle, "%%LanguageLevel: 2\n");
  fwrite($handle, "%%EndComments\n");
  fwrite($handle, "/rf { rectfill } bind def\n");
}

function sunstreaker_write_eps_pixel_run($handle, array $run, int $height): void {
  $x = (int) ($run['x'] ?? 0);
  $y = (int) ($run['y'] ?? 0);
  $w = max(1, (int) ($run['w'] ?? 1));
  $eps_y = max(0, $height - $y - 1);
  $r = max(0, min(255, (int) ($run['r'] ?? 0))) / 255;
  $g = max(0, min(255, (int) ($run['g'] ?? 0))) / 255;
  $b = max(0, min(255, (int) ($run['b'] ?? 0))) / 255;

  fwrite(
    $handle,
    number_format($r, 4, '.', '').' '.
    number_format($g, 4, '.', '').' '.
    number_format($b, 4, '.', '').' setrgbcolor '.
    $x.' '.$eps_y.' '.$w.' 1 rf'."\n"
  );
}

function sunstreaker_write_eps_header($handle, int $width, int $height): void {
  fwrite($handle, "%!PS-Adobe-3.0 EPSF-3.0\n");
  fwrite($handle, "%%Creator: Sunstreaker\n");
  fwrite($handle, "%%BoundingBox: 0 0 ".$width." ".$height."\n");
  fwrite($handle, "%%LanguageLevel: 2\n");
  fwrite($handle, "%%EndComments\n");
  fwrite($handle, "/picstr ".($width * 3)." string def\n");
  fwrite($handle, $width." ".$height." scale\n");
  fwrite($handle, $width." ".$height." 8\n");
  fwrite($handle, "[".$width." 0 0 -".$height." 0 ".$height."]\n");
  fwrite($handle, "{ currentfile picstr readhexstring pop } false 3 colorimage\n");
}

function sunstreaker_write_eps_from_image_with_imagemagick_rgb(string $source_path, string $eps_path) {
  ...
  $command .= ' '.escapeshellarg($source_path).' -alpha set -depth 8 txt:- 2>&1';
  ...
  if ($code !== 0 || empty($output)) {
    return new WP_Error(
      'sunstreaker_eps_pixels_failed',
      trim(implode("\n", array_filter(array_map('strval', $output)))) ?: 'Could not read EPS source pixels.'
    );
  }
  ...
  foreach ($output as $line) {
    $pixel = sunstreaker_parse_imagemagick_txt_pixel((string) $line);
    if (empty($pixel)) continue;

    if (empty($pixel['visible'])) {
      if (is_array($run)) {
        sunstreaker_write_eps_pixel_run($fh, $run, $height);
        $run = null;
      }
...
function sunstreaker_parse_imagemagick_txt_pixel(string $line): array {
  if (!preg_match('/^(\d+),(\d+):\s+(.+)$/', trim($line), $location_matches)) {
    return [];
  }

  $raw = (string) $location_matches[3];
  $pixel = [
    'x' => (int) $location_matches[1],
    'y' => (int) $location_matches[2],
    'r' => 0,
    'g' => 0,
    'b' => 0,
    'visible' => true,
  ...
  if (preg_match('/\((\d+(?:\.\d+)?),(\d+(?:\.\d+)?),(\d+(?:\.\d+)?)(?:,([0-9.]+%?))?\)/', $raw, $rgba_matches)) {
    $pixel['r'] = max(0, min(255, (int) round((float) $rgba_matches[1])));
    $pixel['g'] = max(0, min(255, (int) round((float) $rgba_matches[2])));
    $pixel['b'] = max(0, min(255, (int) round((float) $rgba_matches[3])));
    if (isset($rgba_matches[4]) && $rgba_matches[4] !== '') {
      $alpha_raw = (string) $rgba_matches[4];
      $alpha = (float) rtrim($alpha_raw, '%');
      $pixel['visible'] = substr($alpha_raw, -1) === '%' ? $alpha > 0.0 : $alpha > 0.0;
    }
    return $pixel;
  }

  if (preg_match('/#([0-9A-Fa-f]{6})([0-9A-Fa-f]{2})?/', $raw, $hex_matches)) {
    $hex = (string) $hex_matches[1];
    $pixel['r'] = hexdec(substr($hex, 0, 2));
    $pixel['g'] = hexdec(substr($hex, 2, 2));
    $pixel['b'] = hexdec(substr($hex, 4, 2));
    if (isset($hex_matches[2]) && $hex_matches[2] !== '') {
      $pixel['visible'] = hexdec((string) $hex_matches[2]) > 0;
    }
    return $pixel;
  }
...
function sunstreaker_convert_png_to_webp(string $png_path, string $webp_path, int $quality = 92) {
  if (!is_readable($png_path)) {
    return new WP_Error('sunstreaker_webp_source_missing', 'PNG source not found for WebP conversion.');
  }

  $cwebp = sunstreaker_find_binary_command(['/usr/bin/cwebp', '/usr/local/bin/cwebp', 'cwebp']);
  if ($cwebp !== '') {
    $output = [];
    $code = 0;
    $command = escapeshellcmd($cwebp).' -quiet -q '.max(1, min(100, $quality)).' '.escapeshellarg($png_path).' -o '.escapeshellarg($webp_path).' 2>&1';
    @exec($command, $output, $code);
    if ($code === 0 && file_exists($webp_path)) return $webp_path;
  }
  ...
  if (function_exists('imagecreatefrompng') && function_exists('imagewebp')) {
    ...
    if ($image) {
      ...
      $saved = @imagewebp($image, $webp_path, max(1, min(100, $quality)));
      imagedestroy($image);
      if ($saved && file_exists($webp_path)) return $webp_path;
    }
  }

  return new WP_Error('sunstreaker_webp_convert_failed', 'Could not convert mockup PNG to WebP.');
}
...
function sunstreaker_generate_mockup_assets_for_order($order_id): void {
  ...
  foreach ($order->get_items('line_item') as $item_id => $item) {
    ...
    $existing_webp_url = trim((string) $item->get_meta('_sunstreaker_mockup_webp_url', true));
    $existing_png_url = trim((string) $item->get_meta('_sunstreaker_mockup_png_url', true));
    if ($item->get_meta('_sunstreaker_mockup_source', true) === 'browser' && $existing_png_url !== '') {
      sunstreaker_sync_generated_mockup_meta($item, $existing_png_url);
      $item->update_meta_data('_sunstreaker_mockup_generated', 'yes');
      $item->delete_meta_data('_sunstreaker_mockup_error');
      $item->update_meta_data('_sunstreaker_mockup_generated_at', current_time('mysql'));
      $item->save();
      continue;
    }
    if ($item->get_meta('_sunstreaker_mockup_generated', true) === 'yes' && $existing_webp_url !== '') {
      sunstreaker_sync_generated_mockup_meta($item, sunstreaker_preferred_mockup_display_url($existing_svg_url, $existing_webp_url, $existing_png_url));
      $item->save();
      continue;
    }
    ...
    if (!empty($result['webp_url'])) {
      $item->update_meta_data('_sunstreaker_mockup_webp_url', esc_url_raw($result['webp_url']));
    } else {
      $item->delete_meta_data('_sunstreaker_mockup_webp_url');
    }
    if (!empty($result['png_url'])) {
    ...
    sunstreaker_sync_generated_mockup_meta(
      $item,
      sunstreaker_preferred_mockup_display_url((string) ($result['svg_url'] ?? ''), (string) ($result['webp_url'] ?? ''), (string) ($result['png_url'] ?? ''))
    );
    $item->save();
  }
}
...
function sunstreaker_generate_mockup_asset_for_item($order, int $item_id, $item, array $args) {
  ...
  if ($svg_written === false) {
    return new WP_Error('sunstreaker_mockup_write_svg_failed', 'Could not write mockup SVG file.');
	  ...
	  if (is_wp_error($png_result)) return $png_result;
	  $webp_result = sunstreaker_convert_png_to_webp($paths['png_path'], $paths['webp_path']);
	  if (is_wp_error($webp_result)) return $webp_result;

	  return [
	    'svg_url' => sunstreaker_upload_url_from_path($paths['svg_path']),
	    'webp_url' => sunstreaker_upload_url_from_path($paths['webp_path']),
	    'png_url' => sunstreaker_upload_url_from_path($paths['png_path']),
	  ];
	}
...
function sunstreaker_render_original_art_svg(array $args) {
  ...
  $mockup_text_svg = sunstreaker_render_original_art_svg_from_mockup_text([
    'name' => $name,
    'number' => $number,
    'ink_color' => $ink_color,
    'font_stack' => $font_stack,
  ...
  if ($mockup_text_svg !== '') {
    return [
      'svg' => sunstreaker_embed_font_faces_in_svg($mockup_text_svg, [$font_stack]),
      'width_px' => $width_px,
      'height_px' => $height_px,
    ];
  }
  ...
  foreach ($layers as $layer) {
    ...
    $font_size = (float) ($layer_fit['font_size'] ?? 100);
    $font_weight = (int) ($layer['font_weight'] ?? 700);
    $letter_spacing = (float) ($layer_fit['letter_spacing'] ?? 0);
    ...
    $svg .= '    <text id="'.sunstreaker_xml_escape((string) $layer['id']).'" x="0" y="0" text-anchor="middle" dominant-baseline="middle" fill="'.sunstreaker_xml_escape($ink_color).'" style="'.sunstreaker_xml_escape($layer_style).'">'.sunstreaker_xml_escape((string) $layer['text']).'</text>'."\n";
    $svg .= '  </g>'."\n";
  }
  $svg .= '</svg>'."\n";
...
function sunstreaker_render_original_art_svg_from_mockup_text(array $args): string {
  if (!function_exists('sunstreaker_render_composite_svg')) return '';

  $name = trim((string) ($args['name'] ?? ''));
  $number = trim((string) ($args['number'] ?? ''));
  $reference = isset($args['preview_reference']) && is_array($args['preview_reference']) ? $args['preview_reference'] : [];
  $reference_width = max(1.0, (float) ($reference['width'] ?? 1200));
  $reference_height = max(1.0, (float) ($reference['height'] ?? 1200));
  $crop = isset($args['crop']) && is_array($args['crop']) ? $args['crop'] : [];
  $scale = max(0.000001, (float) ($args['scale'] ?? 1.0));
  $width_px = max(1, (int) ($args['width_px'] ?? 0));
  $height_px = max(1, (int) ($args['height_px'] ?? 0));
  $design_width_in = max(0.01, (float) ($args['design_width_in'] ?? 10.0));
  $height_in = max(0.01, (float) ($args['height_in'] ?? 1.0));
  $crop_left = (float) ($crop['left'] ?? 0.0);
  $crop_top = (float) ($crop['top'] ?? 0.0);

  $mockup_svg = sunstreaker_render_composite_svg([
    'image_url' => '',
    'reference' => [
      'width' => $reference_width,
      'height' => $reference_height,
    ...
    'boundaries' => isset($args['boundaries']) && is_array($args['boundaries']) ? $args['boundaries'] : [],
    'ink_color' => (string) ($args['ink_color'] ?? 'White'),
    'font_stack' => (string) ($args['font_stack'] ?? '"Varsity Block","Freshman","College","Oswald","Arial Black",sans-serif'),
    'name' => $name,
    'number' => $number,
  ...
  if ($mockup_svg === '') return '';

  $expected = [];
  if ($name !== '') {
    $expected[] = sunstreaker_original_art_uppercase(trim((string) preg_replace('/\s+/', ' ', $name)));
  }
  if ($number !== '') {
    $number_text = preg_replace('/[^0-9]/', '', $number);
    $expected[] = $number_text !== '' ? $number_text : trim($number);
  }

  $groups = [];
  $pattern = '~<g\b[^>]*>\s*<text\b[^>]*>.*?</text>\s*</g>~is';
  if (!preg_match_all($pattern, $mockup_svg, $matches)) return '';

  foreach ($matches[0] as $group) {
    $text = html_entity_decode(trim(strip_tags((string) $group)), ENT_QUOTES | ENT_XML1, 'UTF-8');
    $normalized = preg_replace('/\s+/', ' ', $text);
    $normalized = trim((string) $normalized);
    $normalized_upper = sunstreaker_original_art_uppercase($normalized);
    $normalized_number = preg_replace('/[^0-9]/', '', $normalized);
    if ($normalized_number === '') $normalized_number = $normalized;

    foreach ($expected as $candidate) {
      if ($candidate === '') continue;
      if ($normalized_upper === $candidate || $normalized_number === $candidate) {
        $groups[] = $group;
        break;
      }
    }
  ...
  $matrix_x = -$crop_left * $scale;
  $matrix_y = -$crop_top * $scale;
  $svg = '<?xml version="1.0" encoding="UTF-8"?>'."\n";
  $svg .= '<svg xmlns="http://www.w3.org/2000/svg" width="'.number_format($design_width_in, 4, '.', '').'in" height="'.number_format($height_in, 4, '.', '').'in" viewBox="0 0 '.$width_px.' '.$height_px.'">'."\n";
  $svg .= '  <title>Original Art Back</title>'."\n";
  $svg .= '  <desc>Generated from the same Name/Number SVG text used for the Sunstreaker mockup.</desc>'."\n";
  $svg .= '  <rect x="0" y="0" width="'.$width_px.'" height="'.$height_px.'" fill="none" />'."\n";
  $svg .= '  <g transform="matrix('.number_format($scale, 8, '.', '').' 0 0 '.number_format($scale, 8, '.', '').' '.number_format($matrix_x, 4, '.', '').' '.number_format($matrix_y, 4, '.', '').')">'."\n";
