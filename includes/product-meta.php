 ...
 * - _sunstreaker_logo_locations: JSON map of per-location logo settings
 * - _sunstreaker_logo_price: legacy aggregate decimal string (e.g., 14.00)
 * - _sunstreaker_logo_ids: legacy aggregate comma-separated attachment IDs
 ...
 * - _sunstreaker_logos_production: legacy aggregate production method label
...
function sunstreaker_logo_print_locations(): array {
  return [
    'front' => ['label' => 'Front'],
    'back' => ['label' => 'Back'],
    'left_chest' => ['label' => 'Left Chest'],
    'right_chest' => ['label' => 'Right Chest'],
    'left_sleeve' => ['label' => 'Left S.'],
    'right_sleeve' => ['label' => 'Right S.'],
  ];
}

function sunstreaker_get_logo_location_label(string $location_key): string {
  $locations = sunstreaker_logo_print_locations();
  if (!empty($locations[$location_key]['label'])) {
    return (string) $locations[$location_key]['label'];
  }

  return ucwords(str_replace('_', ' ', sanitize_key($location_key)));
}

function sunstreaker_default_logo_location_settings(string $location_key = ''): array {
  $defaults = [
    'enabled' => 'no',
    'price' => '14.00',
...
function sunstreaker_sanitize_logo_location_settings_entry($raw, string $location_key): array {
  $defaults = sunstreaker_default_logo_location_settings($location_key);
  if (!is_array($raw)) $raw = [];

  $enabled = !empty($raw['enabled']) && (string) $raw['enabled'] === 'yes' ? 'yes' : 'no';
  $price = isset($raw['price']) ? (string) $raw['price'] : $defaults['price'];
  $price = preg_replace('/[^0-9.]/', '', $price);
  if ($price === '') $price = (string) $defaults['price'];
  $price = number_format(max(0.0, (float) $price), 2, '.', '');

  $production = sunstreaker_sanitize_production_method(
    $raw['production'] ?? '',
...
function sunstreaker_get_logo_location_settings($product_id): array {
  $product_id = sunstreaker_get_settings_product_id($product_id);
  $locations = sunstreaker_logo_print_locations();
  $settings = [];
  $raw = $product_id > 0 ? get_post_meta($product_id, '_sunstreaker_logo_locations', true) : '';
  ...
  if (is_array($raw) && !empty($raw)) {
    foreach ($locations as $location_key => $location) {
      $settings[$location_key] = sunstreaker_sanitize_logo_location_settings_entry($raw[$location_key] ?? [], $location_key);
    }
    return $settings;
  }

  foreach ($locations as $location_key => $location) {
    $settings[$location_key] = sunstreaker_default_logo_location_settings($location_key);
  }

  if ($product_id > 0 && sunstreaker_uses_logos($product_id)) {
    $legacy_logo_ids = sunstreaker_sanitize_logo_ids(get_post_meta($product_id, '_sunstreaker_logo_ids', true));
    $legacy_price = get_post_meta($product_id, '_sunstreaker_logo_price', true);
    $legacy_production = get_post_meta($product_id, '_sunstreaker_logos_production', true);

    if (!empty($legacy_logo_ids)) {
      $settings['left_chest'] = sunstreaker_sanitize_logo_location_settings_entry([
        'enabled' => 'yes',
        'price' => $legacy_price,
        'production' => $legacy_production,
        'logo_ids' => $legacy_logo_ids,
      ], 'left_chest');
    }
  }
...
function sunstreaker_get_enabled_logo_location_settings($product_id): array {
  $enabled = [];
  foreach (sunstreaker_get_logo_location_settings($product_id) as $location_key => $location_settings) {
    if (($location_settings['enabled'] ?? 'no') !== 'yes') continue;
    $enabled[$location_key] = $location_settings;
  }
  return $enabled;
}

function sunstreaker_get_logo_price($product_id): float {
  foreach (sunstreaker_get_enabled_logo_location_settings($product_id) as $location_settings) {
    $raw = isset($location_settings['price']) ? (string) $location_settings['price'] : '';
    $raw = preg_replace('/[^0-9.]/', '', $raw);
    if ($raw === '') continue;
    return max(0.0, (float) $raw);
  }
...
function sunstreaker_get_logo_ids($product_id): array {
  $location_ids = [];
  foreach (sunstreaker_get_logo_location_settings($product_id) as $location_settings) {
    if (empty($location_settings['logo_ids']) || !is_array($location_settings['logo_ids'])) continue;
    foreach ($location_settings['logo_ids'] as $logo_id) {
      $location_ids[] = absint($logo_id);
    }
  }

  $location_ids = array_values(array_unique(array_filter($location_ids)));
  if (!empty($location_ids)) return $location_ids;
...
function sunstreaker_get_logo_location_ids($product_id, string $location_key): array {
  $settings = sunstreaker_get_logo_location_settings($product_id);
  if (empty($settings[$location_key]['logo_ids']) || !is_array($settings[$location_key]['logo_ids'])) {
    return [];
  }
...
function sunstreaker_default_logo_location_boundaries(): array {
  return [
    'logo_front' => [
      'x' => 0.18,
      'y' => 0.20,
...
function sunstreaker_default_preview_boundaries(): array {
  ...
  ], sunstreaker_default_logo_location_boundaries());
}
...
function sunstreaker_get_preview_boundaries($product_id): array {
  ...
  foreach ($defaults as $key => $fallback) {
    ...
    if (($stored === '' || $stored === null) && strpos($key, 'logo_') === 0) {
      $stored = get_post_meta($product_id, '_sunstreaker_logo_boundary', true);
    }
...
add_action('woocommerce_product_options_general_product_data', function () {
  ...
  $logo_location_settings = sunstreaker_get_logo_location_settings($product_id);
  $logo_locations = sunstreaker_logo_print_locations();
  ...
  woocommerce_wp_checkbox([
    ...
    'desc_tip'    => true,
    'description' => __('Configure allowed print locations and their logo options for this product.', 'sunstreaker'),
  ]);
  echo '<div class="sunstreaker-feature-settings sunstreaker-logo-library-wrap">';
  echo '  <p class="form-field sunstreaker-logo-library__intro">';
  echo '    <label>'.esc_html__('Print Locations', 'sunstreaker').'</label>';
  echo '    <span class="description">'.esc_html__('Check each print location you want available for this garment. Each selected location stores its own price, production type, and allowed logos.', 'sunstreaker').'</span>';
  echo '  </p>';
  echo '  <div class="sunstreaker-logo-locations">';
  foreach ($logo_locations as $location_key => $location) {
    ...
    echo '        <input type="checkbox" class="sunstreaker-logo-location__toggle checkbox" name="_sunstreaker_logo_location_enabled['.esc_attr($location_key).']" value="yes" '.checked(($location_settings['enabled'] ?? 'no'), 'yes', false).' />';
    echo '        <span>'.esc_html($location_label).'</span>';
    echo '      </label>';
    echo '      <div class="sunstreaker-logo-location__settings">';
    echo '        <p class="form-field">';
    echo '          <label for="_sunstreaker_logo_location_price_'.esc_attr($location_key).'">'.esc_html__('Price', 'sunstreaker').'</label>';
    echo '          <input type="text" class="short wc_input_price" id="_sunstreaker_logo_location_price_'.esc_attr($location_key).'" name="_sunstreaker_logo_location_price['.esc_attr($location_key).']" value="'.esc_attr((string) ($location_settings['price'] ?? '14.00')).'" inputmode="decimal" pattern="^[0-9]+(\.[0-9]{1,2})?$" />';
    echo '          <span class="description">'.esc_html__('Extra cost when this print location is selected.', 'sunstreaker').'</span>';
    echo '        </p>';
    echo '        <p class="form-field">';
    echo '          <label for="_sunstreaker_logo_location_production_'.esc_attr($location_key).'">'.esc_html__('Production Type', 'sunstreaker').'</label>';
    echo '          <select id="_sunstreaker_logo_location_production_'.esc_attr($location_key).'" name="_sunstreaker_logo_location_production['.esc_attr($location_key).']">';
    foreach (sunstreaker_production_method_choices() as $production_value => $production_label) {
      echo '            <option value="'.esc_attr($production_value).'" '.selected((string) ($location_settings['production'] ?? ''), (string) $production_value, false).'>'.esc_html((string) $production_label).'</option>';
    }
    echo '          </select>';
    echo '          <span class="description">'.esc_html__('Production method used for this location.', 'sunstreaker').'</span>';
    echo '        </p>';
...
add_action('woocommerce_admin_process_product_object', function ($product) {
  ...
  $logo_locations = sunstreaker_logo_print_locations();
  $posted_logo_location_enabled = isset($_POST['_sunstreaker_logo_location_enabled']) && is_array($_POST['_sunstreaker_logo_location_enabled'])
    ? wp_unslash($_POST['_sunstreaker_logo_location_enabled'])
    : [];
  $posted_logo_location_price = isset($_POST['_sunstreaker_logo_location_price']) && is_array($_POST['_sunstreaker_logo_location_price'])
    ? wp_unslash($_POST['_sunstreaker_logo_location_price'])
    : [];
  $posted_logo_location_production = isset($_POST['_sunstreaker_logo_location_production']) && is_array($_POST['_sunstreaker_logo_location_production'])
    ? wp_unslash($_POST['_sunstreaker_logo_location_production'])
    : [];
  ...
  $aggregate_logo_ids = [];
  $aggregate_price = '14.00';
  $aggregate_production = sunstreaker_default_feature_production_method('logos');
  $has_aggregate = false;

  foreach ($logo_locations as $location_key => $location) {
    $logo_location_settings[$location_key] = sunstreaker_sanitize_logo_location_settings_entry([
      'enabled' => isset($posted_logo_location_enabled[$location_key]) ? 'yes' : 'no',
    ...
    if (!$has_aggregate && $logo_location_settings[$location_key]['enabled'] === 'yes') {
      $aggregate_price = (string) $logo_location_settings[$location_key]['price'];
      $aggregate_production = (string) $logo_location_settings[$location_key]['production'];
      $has_aggregate = true;
    }
  }

  $aggregate_logo_ids = array_values(array_unique(array_filter($aggregate_logo_ids)));
  $product->update_meta_data('_sunstreaker_logo_locations', wp_json_encode($logo_location_settings));
  $product->update_meta_data('_sunstreaker_logo_price', $aggregate_price);
  $product->update_meta_data('_sunstreaker_logos_production', $aggregate_production);
  $product->update_meta_data('_sunstreaker_logo_ids', implode(',', $aggregate_logo_ids));
...
add_action('admin_enqueue_scripts', function($hook){
  ...
  wp_localize_script('sunstreaker-product-edit', 'sunstreakerProductEdit', [
    ...
    'selectedLogosByLocation' => $product_id > 0 ? array_reduce(array_keys(sunstreaker_logo_print_locations()), static function(array $carry, string $location_key) use ($product_id): array {
