(function () {
  ...
  var state = {
    ...
    drag: null,
    editing: false,
    generatedAssetSubmitting: false,
    generatedAssetPreparing: false,
    debug: debugMode,
  ...
  function mockupBaseFilename() {
    return mockupFilename('svg').replace(/\.svg$/i, '');
  }
  function fileExtensionFromType(type) {
    ...
    if (type.indexOf('image/svg') === 0) return 'svg';
    return 'png';
  }
  ...
  function fileFromBlob(blob, filename) {
    if (typeof File === 'function') return new File([blob], filename, { type: blob.type || 'application/octet-stream' });
    blob.name = filename;
    return blob;
  }
  ...
  function inlineSvgImageAssets(svg) {
    var parser;
    var doc;
    var serializer;
    var images;
    ...
    if (!window.DOMParser || !window.XMLSerializer) return Promise.resolve(svg);

    parser = new DOMParser();
    doc = parser.parseFromString(svg, 'image/svg+xml');
    if (doc.querySelector('parsererror')) return Promise.resolve(svg);

    images = Array.prototype.slice.call(doc.querySelectorAll('image'));
    images.forEach(function (image) {
      var href = image.getAttribute('href') || image.getAttribute('xlink:href') || '';
      if (!href || href.indexOf('data:') === 0 || href.indexOf('blob:') === 0 || urls.indexOf(href) !== -1) return;
      urls.push(href);
    });

    if (!urls.length) return Promise.resolve(svg);

    return Promise.all(urls.map(function (url) {
      return blobFromImageUrl(url).then(blobToDataUrl).then(function (dataUrl) {
      ...
      replacements.forEach(function (replacement) {
        images.forEach(function (image) {
          var href = image.getAttribute('href') || image.getAttribute('xlink:href') || '';
          if (href !== replacement.url) return;
          image.setAttribute('href', replacement.dataUrl);
          image.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', replacement.dataUrl);
        });
      });
  ...
  function pngBlobFromSvg(svg, reference) {
    return inlineSvgImageAssets(svg).catch(function () {
      return svg;
    }).then(function (rasterSvg) {
      return new Promise(function (resolve, reject) {
        ...
        var canvas;
        var context;
        var width = Math.max(1, Math.round(reference && reference.width ? reference.width : 0));
        var height = Math.max(1, Math.round(reference && reference.height ? reference.height : 0));

        if (!url || !document.createElement('canvas').getContext) {
          reject(new Error('PNG export is not available.'));
          return;
        }
        ...
        image.onerror = function () {
          window.URL.revokeObjectURL(url);
          reject(new Error('Mockup image rendering failed.'));
        };
        image.src = url;
      });
    });
  ...
  function imageFromDataUrl(dataUrl) {
    return new Promise(function (resolve, reject) {
      ...
      image.onerror = function () { reject(new Error('Image rendering failed.')); };
      image.src = dataUrl;
    });
  }
  function imageFromUrl(url) {
    if (!url) return Promise.reject(new Error('Image source is missing.'));
    return blobFromImageUrl(url).then(blobToDataUrl).then(imageFromDataUrl);
  }
  function relativeRect(rect, frameRect, scaleX, scaleY) {
    return {
      x: (rect.left - frameRect.left) * scaleX,
      y: (rect.top - frameRect.top) * scaleY,
      w: rect.width * scaleX,
      h: rect.height * scaleY
    };
  }
  function drawContainImage(context, image, box) {
    var imageW = image.naturalWidth || image.width || 0;
    var imageH = image.naturalHeight || image.height || 0;
    var scale;
    var w;
    var h;
    var x;
    ...
    scale = Math.min(box.w / imageW, box.h / imageH);
    w = imageW * scale;
    h = imageH * scale;
    x = box.x + ((box.w - w) / 2);
    y = box.y + ((box.h - h) / 2);
  ...
  function drawCoverImage(context, image, box) {
    var imageW = image.naturalWidth || image.width || 0;
    var imageH = image.naturalHeight || image.height || 0;
    var scale;
    var w;
    ...
    if (!imageW || !imageH || !box.w || !box.h) return;
    scale = Math.max(box.w / imageW, box.h / imageH);
    w = imageW * scale;
    h = imageH * scale;
    x = box.x + ((box.w - w) / 2);
    y = box.y + ((box.h - h) / 2);
  ...
  function drawImageElement(context, imageEl, frameRect, scaleX, scaleY, fit) {
    var url = imageEl.currentSrc || imageEl.src || imageEl.getAttribute('src') || '';
    var rect = imageEl.getBoundingClientRect();
    var box;

    if (!url || imageEl.hidden || rect.width <= 0 || rect.height <= 0) return Promise.resolve();
    box = relativeRect(rect, frameRect, scaleX, scaleY);
    return imageFromUrl(url).then(function (image) {
      if (fit === 'cover') drawCoverImage(context, image, box);
      else if (fit === 'stretch') context.drawImage(image, box.x, box.y, box.w, box.h);
      else drawContainImage(context, image, box);
    });
  }
  function parseScaleFromTransform(transform) {
    ...
    if (match) {
      ...
      if (parts.length >= 4) return { x: Number.isFinite(parts[0]) ? parts[0] : 1, y: Number.isFinite(parts[3]) ? parts[3] : 1 };
    }
    match = transform.match(/^matrix3d\(([^)]+)\)$/);
    if (match) {
      parts = match[1].split(',').map(function (part) { return Number(part.trim()); });
      if (parts.length >= 16) return { x: Number.isFinite(parts[0]) ? parts[0] : 1, y: Number.isFinite(parts[5]) ? parts[5] : 1 };
    }
    return { x: 1, y: 1 };
  }
  function textForCanvas(el, style) {
    var value = el ? String(el.textContent || '') : '';
    var transform = style && style.textTransform ? String(style.textTransform).toLowerCase() : '';
    if (transform === 'uppercase') return value.toUpperCase();
    if (transform === 'lowercase') return value.toLowerCase();
    return value;
  }
  function drawSpacedText(context, text, letterSpacing) {
    ...
    for (i = 0; i < text.length; i += 1) {
      ...
      context.translate(context.measureText(text.charAt(i)).width + spacing, 0);
    }
  }
  function drawTextElement(context, valueEl, frameRect, scaleX, scaleY, options) {
    var rect;
    var style;
    var parentStyle;
    var text;
    ...
    if (!valueEl || valueEl.hidden) return;
    rect = valueEl.getBoundingClientRect();
    if (rect.width <= 0 || rect.height <= 0) return;
    style = window.getComputedStyle(valueEl);
    parentStyle = valueEl.parentElement ? window.getComputedStyle(valueEl.parentElement) : style;
    text = textForCanvas(valueEl, parentStyle);
    if (!text) return;

    scale = parseScaleFromTransform(style.transform);
    fontSize = (parseFloat(style.fontSize) || 16) * scaleY;
    fontWeight = parentStyle.fontWeight || style.fontWeight || '400';
    fontStyle = parentStyle.fontStyle || style.fontStyle || 'normal';
    fontFamily = parentStyle.fontFamily || style.fontFamily || 'sans-serif';
    ...
    context.scale(scale.x || 1, scale.y || 1);
    context.font = fontStyle + ' ' + fontWeight + ' ' + fontSize + 'px ' + fontFamily;
    context.fillStyle = parentStyle.color || style.color || '#fff';
    context.textAlign = 'center';
    context.textBaseline = 'middle';
    if (!options || !options.noShadow) {
      context.shadowColor = 'rgba(0,0,0,0.45)';
      context.shadowBlur = 2 * Math.max(scaleX, scaleY);
      context.shadowOffsetY = 1 * scaleY;
    }
    drawSpacedText(context, text, letterSpacing * scaleX);
    context.restore();
  }
  function drawRenderedPreviewText(context, frameRect, scaleX, scaleY) {
    ['name', 'number'].forEach(function (field) {
      var el = state.els[field];
      if (!el || el.classList.contains('is-hidden') || el.classList.contains('is-empty')) return;
      drawTextElement(context, el.querySelector('.sunstreaker-boundary__value'), frameRect, scaleX, scaleY);
    });

    if (state.els.right_chest && !state.els.right_chest.classList.contains('is-hidden') && !state.els.right_chest.classList.contains('is-empty')) {
      drawTextElement(context, state.els.right_chest.querySelector('.sunstreaker-boundary__right-chest-line--name .sunstreaker-boundary__right-chest-value'), frameRect, scaleX, scaleY);
      drawTextElement(context, state.els.right_chest.querySelector('.sunstreaker-boundary__right-chest-line--department .sunstreaker-boundary__right-chest-value'), frameRect, scaleX, scaleY);
    }
  }
  function renderedMockupBlob() {
    var reference = currentPreviewReference();
    ...
    if (!frame || !image) return Promise.reject(new Error('Mockup preview is missing.'));
    frameRect = frame.getBoundingClientRect();
    if (!frameRect.width || !frameRect.height) return Promise.reject(new Error('Mockup preview is empty.'));

    canvas = document.createElement('canvas');
    canvas.width = Math.max(1, Math.round(reference.width || frameRect.width));
    canvas.height = Math.max(1, Math.round(reference.height || frameRect.height));
    context = canvas.getContext('2d');
    if (!context) return Promise.reject(new Error('PNG export is not available.'));

    scaleX = canvas.width / frameRect.width;
    scaleY = canvas.height / frameRect.height;
    baseUrl = image.getAttribute('data-large_image') || image.currentSrc || image.getAttribute('src') || '';
    logoImages = state.stage ? Array.prototype.slice.call(state.stage.querySelectorAll('.sunstreaker-boundary__image')) : [];
    frontBackImages = frame ? Array.prototype.slice.call(frame.querySelectorAll('.sunstreaker-front-back__image')) : [];
    fontsReady = document.fonts && document.fonts.ready ? document.fonts.ready : Promise.resolve();

    return imageFromUrl(baseUrl).then(function (baseImage) {
      context.drawImage(baseImage, 0, 0, canvas.width, canvas.height);
      return Promise.all(logoImages.map(function (imageEl) {
        return drawImageElement(context, imageEl, frameRect, scaleX, scaleY, 'contain');
      }));
    }).then(function () {
      return fontsReady;
    }).then(function () {
      drawRenderedPreviewText(context, frameRect, scaleX, scaleY);
      return Promise.all(frontBackImages.map(function (imageEl) {
        return drawImageElement(context, imageEl, frameRect, scaleX, scaleY, 'contain');
      }));
    }).then(function () {
  ...
  function cropTransparentCanvas(source) {
    ...
    croppedContext = cropped.getContext('2d');
  ...
  function renderedBackOriginalArtBlob() {
    var reference = currentPreviewReference();
    var frame = state.frame;
    var frameRect;
    var canvas;
    ...
    var scaleY;
    var fontsReady;
    var hasName = previewValue('name') !== '';
    var hasNumber = previewValue('number') !== '';

    if (!hasName || !hasNumber) return Promise.resolve(null);
    if (!frame) return Promise.resolve(null);
    frameRect = frame.getBoundingClientRect();
    if (!frameRect.width || !frameRect.height) return Promise.resolve(null);
    ...
    return fontsReady.then(function () {
      ['name', 'number'].forEach(function (field) {
        var el = state.els[field];
        if (!el || el.classList.contains('is-hidden') || el.classList.contains('is-empty')) return;
        drawTextElement(context, el.querySelector('.sunstreaker-boundary__value'), frameRect, scaleX, scaleY, { noShadow: true });
      });
      return blobFromCanvas(cropTransparentCanvas(canvas));
    });
  }
  function generatedAssetInput(kind) {
    if (kind === 'mockup') return document.getElementById('sunstreaker_mockup_png_url');
    if (kind === 'original_back') return document.getElementById('sunstreaker_original_art_back_png_url');
    return null;
  }
  function uploadGeneratedAsset(kind, blob, retried) {
    var body;
    var filename;
    if (!blob) return Promise.resolve('');
    body = new FormData();
    filename = kind === 'original_back' ? 'original-art-back.png' : mockupFilename('png');
    body.append('action', 'sunstreaker_upload_generated_mockup_asset');
    ...
    return fetch(boundaryAjaxUrl(), {
      ...
      return response.json().catch(function () {
        throw new Error('Generated asset upload returned an invalid response (HTTP ' + response.status + ').');
      }).then(function (payload) {
        if (!response.ok || !payload || !payload.success || !payload.data || !payload.data.url) {
          var message = payload && payload.data && payload.data.message ? String(payload.data.message) : 'Generated asset upload failed (HTTP ' + response.status + ').';
          var error = new Error(message);
          error.expiredNonce = response.status === 403 && message === 'Bad nonce';
          throw error;
        }
        return payload;
      });
      ...
      var input;
      input = generatedAssetInput(kind);
      if (input) input.value = String(payload.data.url);
      return String(payload.data.url);
    }).catch(function (error) {
      if (!retried && error.expiredNonce) {
        return boundaryRequest('sunstreaker_mockup_nonce').then(parseResponse).then(function (data) {
          if (!data.nonce) throw new Error('Mockup nonce refresh failed.');
          settings.nonce = String(data.nonce);
          return uploadGeneratedAsset(kind, blob, true);
        }).catch(function (refreshError) {
          refreshError.mockupUpload = true;
          throw refreshError;
        });
      }
      error.mockupUpload = true;
      throw error;
    });
  }
  function hasGeneratedMockupData() {
    if (previewValue('name') !== '' || previewValue('number') !== '') return true;
    if (rightChestPreviewValues().name !== '' || rightChestPreviewValues().department !== '') return true;
    if (selectedLogoCompositeEntries().length > 0) return true;
    if (useFrontBack) {
      if ((artUrlInput('front') && artUrlInput('front').value) || (artUrlInput('back') && artUrlInput('back').value)) return true;
    }
    return false;
  }
  function prepareGeneratedAssetsForSubmit() {
    if (!hasGeneratedMockupData()) return Promise.resolve();
    return renderedMockupBlob().then(function (mockupBlob) {
      return uploadGeneratedAsset('mockup', mockupBlob);
    }).then(function () {
      return renderedBackOriginalArtBlob();
    }).then(function (originalBackBlob) {
      return uploadGeneratedAsset('original_back', originalBackBlob);
    });
  }
  ...
  function uniqueRosterCardElements() {
    var selectors = [
      '[data-sunstreaker-roster-card-image]',
      '[data-sunstreaker-team-card-image]',
      '[data-sunstreaker-roster-card] img',
      '[data-sunstreaker-team-card] img',
      '.sunstreaker-roster-card img',
      '.sunstreaker-team-card img',
      '[data-sunstreaker-roster-card] canvas',
      '[data-sunstreaker-team-card] canvas',
      '.sunstreaker-roster-card canvas',
      '.sunstreaker-team-card canvas'
    ];
    var seen = [];
    return Array.prototype.slice.call(document.querySelectorAll(selectors.join(','))).filter(function (el) {
      var key = el.tagName === 'CANVAS' ? el : (el.currentSrc || el.src || el.getAttribute('href') || el.getAttribute('data-src') || '');
      if (!key || seen.indexOf(key) !== -1) return false;
      seen.push(key);
      return true;
    });
  }
  function rosterCardFilename(el, index, extension) {
    var label = el.getAttribute('data-filename') || el.getAttribute('alt') || el.getAttribute('aria-label') || '';
    var card = el.closest('[data-sunstreaker-roster-card], [data-sunstreaker-team-card], .sunstreaker-roster-card, .sunstreaker-team-card');
    if (!label && card) {
      label = card.getAttribute('data-filename') || card.getAttribute('data-player-name') || card.getAttribute('aria-label') || '';
    }
    label = String(label || ('roster-card-' + index)).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || ('roster-card-' + index);
    return mockupBaseFilename() + '-' + label + '.' + extension;
  }
  function rosterCardFileFromElement(el, index) {
    var url;
    var extension;

    if (el.tagName === 'CANVAS') {
      return blobFromCanvas(el).then(function (blob) {
        return fileFromBlob(blob, rosterCardFilename(el, index, 'png'));
      });
    }

    url = el.currentSrc || el.src || el.getAttribute('href') || el.getAttribute('data-src') || '';
    if (!url) return Promise.reject(new Error('Roster card image is missing.'));
    return blobFromImageUrl(url).then(function (blob) {
      extension = fileExtensionFromUrl(url) || fileExtensionFromType(blob.type);
      return fileFromBlob(blob, rosterCardFilename(el, index, extension));
    });
  }
  function saveFiles(files, successText) {
    var canShareFiles = files.length > 1 && navigator.share && navigator.canShare && navigator.canShare({ files: files });

    if (canShareFiles) {
      return navigator.share({
        files: files,
        title: document.title || 'Sunstreaker mockups'
      }).then(function () {
        setStatus(successText || statusText('sharedMockups', 'Mockups ready to save.'), false);
      });
    }
    ...
    files.forEach(function (file, index) {
      window.setTimeout(function () {
        ...
        triggerDownload(url, file.name || ('mockup-' + (index + 1) + '.' + fileExtensionFromType(file.type)));
        window.setTimeout(function () {
          window.URL.revokeObjectURL(url);
        }, 1000);
      }, index * 250);
    });
    setStatus(successText || statusText('savedMockups', 'Mockups downloaded.'), false);
  ...
  function onSaveMockupClick() {
    ...
    renderedMockupBlob().then(function (blob) {
      return saveFiles([fileFromBlob(blob, mockupFilename('png'))], statusText('savedMockup', 'Mockup downloaded.'));
    }).then(function () {
  ...
  function previewValue(field) {
    ...
    return field === 'number'
      ? previewDefaultValue('number', '')
      : previewDefaultValue('name', '');
  }
  ...
  function previewStageScale() {
    var style;
    var matrix;

    if (!state.stage) return { x: 1, y: 1 };

    style = window.getComputedStyle(state.stage);
    matrix = String(style.transform || '');
    if (!matrix || matrix === 'none') return { x: 1, y: 1 };

    matrix = matrix.match(/matrix3d\(([^)]+)\)/) || matrix.match(/matrix\(([^)]+)\)/);
    if (!matrix) return { x: 1, y: 1 };

    matrix = matrix[1].split(',');
    if (matrix.length >= 16) {
      return {
        x: Math.abs(parseFloat(matrix[0] || '1')) || 1,
        y: Math.abs(parseFloat(matrix[5] || '1')) || 1
      };
    }

    return {
      x: Math.abs(parseFloat(matrix[0] || '1')) || 1,
      y: Math.abs(parseFloat(matrix[3] || '1')) || 1
    };
  }

  function unzoomedRect(rect, scale) {
    scale = scale || { x: 1, y: 1 };
    return {
      width: rect.width / (scale.x || 1),
      height: rect.height / (scale.y || 1)
    };
  }
  ...
  function fitBoundaryText(valueEl, field, maxWidth, maxHeight) {
    ...
    var rect, fittedRect, scaleX, scaleY, attempts, measurementValue, fitBuffer, isRightChestField, widthRatio, heightRatio, uniformScale, minHeightPx, overflow, stageScale;
    ...
    measurementValue = field === 'number'
      ...
      : valueEl.textContent;
    stageScale = previewStageScale();
    rect = unzoomedRect(measureBoundaryTextRect(valueEl, measurementValue), stageScale);
    ...
    for (attempts = 0; attempts < 3; attempts += 1) {
      fittedRect = unzoomedRect(valueEl.getBoundingClientRect(), stageScale);
  ...
  function updateButtons() {
    ...
    if (state.mockupButton) state.mockupButton.hidden = !settings.canSaveMockup;
  ...
  function ensureControls() {
    ...
    if (settings.canEdit) holder.appendChild(primary);
    if (settings.canSaveMockup) holder.appendChild(mockup);
    if (settings.canEdit) holder.appendChild(cancel);
  ...
  function bindGeneratedAssetSubmit() {
    var form = document.querySelector('form.cart');
    if (!form) return;
    form.addEventListener('submit', function (event) {
      var submitter = event.submitter || null;
      if (state.generatedAssetSubmitting) {
        state.generatedAssetSubmitting = false;
        return;
      }
      if (state.generatedAssetPreparing) {
        event.preventDefault();
        return;
      }
      if (!hasGeneratedMockupData()) return;

      event.preventDefault();
      state.generatedAssetPreparing = true;
      setStatus(statusText('savingMockup', 'Preparing mockup...'), false);
      prepareGeneratedAssetsForSubmit().then(function () {
        state.generatedAssetPreparing = false;
        state.generatedAssetSubmitting = true;
        setStatus('', false);
        ...
        state.generatedAssetPreparing = false;
        state.generatedAssetSubmitting = false;
        if (window.console && window.console.error) window.console.error('Sunstreaker mockup preparation failed:', error);
        setStatus(error && error.mockupUpload
          ? statusText('uploadMockupError', 'Could not save your personalized preview. Please try adding the item again.')
          : statusText('saveMockupError', 'Could not generate the mockup.'), true);
      });
    });
  }
