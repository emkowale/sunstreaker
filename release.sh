...
if [[ "$GIT_OK" -eq 1 ]] && command -v gh >/dev/null 2>&1; then
  ...
  if grep -Eqi 'token .*invalid|failed to log in|not logged' <<<"$GH_AUTH_STATUS"; then
    ...
    if gh release view "v${NEXT}" >/dev/null 2>&1; then
      ...
      if gh release upload "v${NEXT}" "${ART_DIR}/${ZIP_NAME}" "$UPDATE_MANIFEST" --clobber >/dev/null; then
        RELEASE_OK=1
      else
        warn "Could not upload release asset"
      fi
    else
      if gh release create "v${NEXT}" "${ART_DIR}/${ZIP_NAME}" "$UPDATE_MANIFEST" -t "v${NEXT}" -n "Release ${NEXT}" >/dev/null; then
        RELEASE_OK=1
      else
