...
add_action('woocommerce_before_add_to_cart_button', function(){
  ...
  $posted_logo_id = function_exists('sunstreaker_get_posted_logo_id') ? sunstreaker_get_posted_logo_id() : 0;
  ...
  $logo_price = function_exists('sunstreaker_get_logo_price') ? sunstreaker_get_logo_price($product_id) : 14.00;
  ...
  $logo_choices = function_exists('sunstreaker_get_logo_choices') ? sunstreaker_get_logo_choices($product_id) : [];
  ...
  $logo_price_label = function_exists('wc_price')
    ? wc_price($logo_price)
    : '$'.number_format($logo_price, 2, '.', '');
  ...
  $logo_note_html = sprintf(
    __('Choose a logo from the drop-down list and have it embroidered on this garment for %s.', 'sunstreaker'),
    $logo_price_label
  );
  ...
  if ($use_logos) {
    if ($posted_logo_id > 0 && function_exists('sunstreaker_get_logo_choice')) {
      if (empty(sunstreaker_get_logo_choice($product_id, $posted_logo_id))) {
        $posted_logo_id = 0;
      }
    }
    ...
    echo '  <div class="sunstreaker-field">';
    echo '    <label for="sunstreaker_logo_id">Logo</label>';
    echo '    <select class="sunstreaker-select" id="sunstreaker_logo_id" name="sunstreaker_logo_id"'.(empty($logo_choices) ? ' disabled' : '').'>';
    echo '      <option value="">'.esc_html__('Choose a logo', 'sunstreaker').'</option>';

    foreach ($logo_choices as $logo) {
      $logo_id = isset($logo['id']) ? (int) $logo['id'] : 0;
      if ($logo_id <= 0) continue;
      $title = isset($logo['title']) ? (string) $logo['title'] : 'Logo '.$logo_id;
      echo '      <option value="'.esc_attr((string) $logo_id).'" '.selected($posted_logo_id, $logo_id, false).'>'.esc_html($title).'</option>';
    }
