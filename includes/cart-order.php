...
add_filter('woocommerce_add_to_cart_validation', function($passed, $product_id, $qty){
  ...
  $logo_ids = (function_exists('sunstreaker_uses_logos') && sunstreaker_uses_logos($product_id) && function_exists('sunstreaker_get_posted_logo_ids'))
    ? sunstreaker_get_posted_logo_ids()
    : [];
  ...
  foreach ($logo_ids as $logo_id) {
    if (!function_exists('sunstreaker_is_allowed_logo') || !sunstreaker_is_allowed_logo($product_id, (int) $logo_id)) {
...
add_filter('woocommerce_add_cart_item_data', function($cart_item_data, $product_id, $variation_id){
  ...
  $logo_choices = [];
  if (function_exists('sunstreaker_get_logo_choice')) {
    foreach ($logo_ids as $logo_id) {
      $choice = sunstreaker_get_logo_choice($product_id, (int) $logo_id);
      if (!empty($choice)) $logo_choices[] = $choice;
    }
  }
  ...
  $logo_addon = !empty($logo_choices) && function_exists('sunstreaker_get_logo_price')
    ? sunstreaker_get_logo_price($product_id)
    : (!empty($logo_choices) ? 14.0 : 0.0);
  ...
  if (!empty($logo_choices)) {
    ...
    $cart_item_data['sunstreaker']['logo_labels'] = array_values(array_map('strval', array_column($logo_choices, 'title')));
  ...
  $cart_item_data['sunstreaker_key'] = md5($name.'|'.$num.'|'.$font_choice.'|'.$right_chest_name.'|'.$right_chest_department.'|'.$right_chest_font_choice.'|'.implode(',', $logo_ids).'|'.wp_json_encode($scrub_values).'|'.microtime(true));
