...
add_action('wp_enqueue_scripts', function(){
  ...
  wp_localize_script('sunstreaker-frontend', 'sunstreakerPreview', [
    ...
    'canEdit' => function_exists('sunstreaker_can_edit_product_boundaries')
      ...
      : false,
    'canSaveMockup' => function_exists('sunstreaker_can_edit_product_boundaries')
      ? sunstreaker_can_edit_product_boundaries($product_id)
      : false,
    'inkColor' => $ink_color,
    ...
    'strings' => [
      ...
      'savedMockups' => __('Mockups downloaded.', 'sunstreaker'),
      'sharedMockups' => __('Mockups ready to save.', 'sunstreaker'),
      'saveMockupError' => __('Could not generate the mockup.', 'sunstreaker'),
      'uploadMockupError' => __('Could not save your personalized preview. Please try adding the item again.', 'sunstreaker'),
...
add_action('woocommerce_before_add_to_cart_button', function(){
  ...
  if (!$mockup_builder_mode && $use_name_number) {
    ...
    echo '    <input type="text" class="sunstreaker-text-input input-text text" id="sunstreaker_name" name="sunstreaker_name" value="'.$posted_name.'" maxlength="20" />';
    echo '  </div>';
    echo '  <div class="sunstreaker-field">';
  ...
  echo '<input type="hidden" id="sunstreaker_mockup_png_url" name="sunstreaker_mockup_png_url" value="" />';
  ...
  if (!$mockup_builder_mode && $use_front_back) {
    ...
    foreach ([
      ...
      $thumb_markup = $config['url'] !== ''
        ? '<img class="sunstreaker-art-upload__status-thumb" src="'.esc_url($config['url']).'" alt="" loading="eager" decoding="async" />'
        : '';
...
function sunstreaker_front_back_art_pixel_rgba($image, int $x, int $y): array {
  $rgba = imagecolorat($image, $x, $y);
  if (function_exists('imageistruecolor') && !imageistruecolor($image)) {
    $color = imagecolorsforindex($image, $rgba);
    return [
      'r' => isset($color['red']) ? (int) $color['red'] : 0,
      'g' => isset($color['green']) ? (int) $color['green'] : 0,
      'b' => isset($color['blue']) ? (int) $color['blue'] : 0,
      'a' => isset($color['alpha']) ? (int) $color['alpha'] : 0,
    ];
  }
...
function sunstreaker_front_back_art_color_close(array $a, array $b, int $threshold): bool {
  return abs((int) $a['r'] - (int) $b['r']) <= $threshold
    && abs((int) $a['g'] - (int) $b['g']) <= $threshold
    && abs((int) $a['b'] - (int) $b['b']) <= $threshold;
}

function sunstreaker_front_back_art_background_color($image, int $width, int $height): ?array {
  $threshold = 32;
  $corners = [
    sunstreaker_front_back_art_pixel_rgba($image, 0, 0),
    sunstreaker_front_back_art_pixel_rgba($image, max(0, $width - 1), 0),
    sunstreaker_front_back_art_pixel_rgba($image, 0, max(0, $height - 1)),
    sunstreaker_front_back_art_pixel_rgba($image, max(0, $width - 1), max(0, $height - 1)),
  ];
  $best = null;
  $best_count = 0;

  foreach ($corners as $candidate) {
    if ((int) $candidate['a'] >= 120) continue;
    $count = 0;
    foreach ($corners as $corner) {
      if ((int) $corner['a'] < 120 && sunstreaker_front_back_art_color_close($candidate, $corner, $threshold)) {
        $count++;
      }
    }
    if ($count > $best_count) {
  ...
  return $best_count >= 3 ? $best : null;
}

function sunstreaker_front_back_shell_exec_available(): bool {
  if (!function_exists('shell_exec')) return false;
  $disabled = array_map('trim', explode(',', (string) ini_get('disable_functions')));
  return !in_array('shell_exec', $disabled, true);
}

function sunstreaker_front_back_convert_binary(): string {
  if (!sunstreaker_front_back_shell_exec_available()) return '';
  foreach (['/usr/bin/convert', '/usr/local/bin/convert', 'convert'] as $candidate) {
    $check = $candidate === 'convert'
...
function sunstreaker_trim_front_back_art_upload_with_imagemagick(array $upload): array {
  $file = isset($upload['file']) ? (string) $upload['file'] : '';
  $type = isset($upload['type']) ? strtolower((string) $upload['type']) : '';
  $convert = sunstreaker_front_back_convert_binary();
  $before = null;
  $after = null;
  ...
  if ($file === '' || !is_readable($file) || $convert === '') return $upload;
  if ($type !== 'image/png' && $type !== 'image/webp' && $type !== 'image/jpeg' && $type !== 'image/jpg') return $upload;
  if ($extension === '') $extension = $type === 'image/png' ? 'png' : ($type === 'image/webp' ? 'webp' : 'jpg');

  $before = @getimagesize($file);
  if (empty($before[0]) || empty($before[1])) return $upload;

  $tmp = tempnam(dirname($file), 'ss-trim-');
  if (!$tmp) return $upload;
  @unlink($tmp);
  $tmp .= '.'.$extension;

  $cmd = escapeshellcmd($convert).' '.escapeshellarg($file).' -fuzz 3% -trim +repage '.escapeshellarg($tmp).' 2>&1';
  @shell_exec($cmd);
  $after = is_readable($tmp) ? @getimagesize($tmp) : null;
  if (empty($after[0]) || empty($after[1])) {
    if (is_file($tmp)) @unlink($tmp);
    return $upload;
  }

  if ((int) $after[0] === (int) $before[0] && (int) $after[1] === (int) $before[1]) {
    @unlink($tmp);
    return $upload;
  }
  ...
  $upload['sunstreaker_trim'] = [
    'x' => 0,
    'y' => 0,
    'width' => (int) $after[0],
    'height' => (int) $after[1],
    'original_width' => (int) $before[0],
    'original_height' => (int) $before[1],
  ];

  return $upload;
}

function sunstreaker_trim_front_back_art_upload(array $upload): array {
  $file = isset($upload['file']) ? (string) $upload['file'] : '';
  $type = isset($upload['type']) ? strtolower((string) $upload['type']) : '';
  $image = null;
  $width = 0;
  ...
  if ($file === '' || !is_readable($file)) return $upload;
  if ($type !== 'image/png' && $type !== 'image/webp' && $type !== 'image/jpeg' && $type !== 'image/jpg') return $upload;
  if (!function_exists('imagecreatetruecolor')) return sunstreaker_trim_front_back_art_upload_with_imagemagick($upload);
  if ($type === 'image/png' && !function_exists('imagecreatefrompng')) return $upload;
  if ($type === 'image/webp' && !function_exists('imagecreatefromwebp')) return $upload;
  if (($type === 'image/jpeg' || $type === 'image/jpg') && !function_exists('imagecreatefromjpeg')) return $upload;

  $size = @getimagesize($file);
  $width = isset($size[0]) ? (int) $size[0] : 0;
  $height = isset($size[1]) ? (int) $size[1] : 0;
  if ($width <= 1 || $height <= 1 || ($width * $height) > 20000000) return $upload;
  ...
  $background = sunstreaker_front_back_art_background_color($image, $width, $height);

  for ($y = 0; $y < $height; $y++) {
    for ($x = 0; $x < $width; $x++) {
      $color = sunstreaker_front_back_art_pixel_rgba($image, $x, $y);
      if ((int) $color['a'] >= 120) {
  ...
  for ($y = 0; $y < $height; $y++) {
    for ($x = 0; $x < $width; $x++) {
      $color = sunstreaker_front_back_art_pixel_rgba($image, $x, $y);
      if ($has_transparency) {
        if ((int) $color['a'] >= 120) continue;
      } else {
        if (sunstreaker_front_back_art_color_close($color, $background, 32)) continue;
      }
      if ($x < $min_x) $min_x = $x;
      if ($y < $min_y) $min_y = $y;
      if ($x > $max_x) $max_x = $x;
  ...
  imagecopy($cropped, $image, 0, 0, $min_x, $min_y, $crop_w, $crop_h);

  $saved = false;
  if ($type === 'image/png' && function_exists('imagepng')) {
    $saved = imagepng($cropped, $file);
  } elseif ($type === 'image/webp' && function_exists('imagewebp')) {
    $saved = imagewebp($cropped, $file, 90);
  } elseif (($type === 'image/jpeg' || $type === 'image/jpg') && function_exists('imagejpeg')) {
...
function sunstreaker_handle_front_back_art_upload() {
  ...
  $upload = sunstreaker_trim_front_back_art_upload($upload);
  ...
  wp_send_json_success([
    ...
    'width' => isset($size[0]) ? (int) $size[0] : 0,
    'height' => isset($size[1]) ? (int) $size[1] : 0,
    'trim' => isset($upload['sunstreaker_trim']) ? $upload['sunstreaker_trim'] : null,
  ]);
}
...
function sunstreaker_handle_mockup_nonce() {
  $product_id = isset($_POST['product_id']) ? absint(wp_unslash($_POST['product_id'])) : 0;
  if ($product_id <= 0 || get_post_type($product_id) !== 'product' || !sunstreaker_is_enabled_for_product($product_id)) {
    wp_send_json_error(['message' => 'Invalid product'], 400);
  }

  // The public product page already exposes this nonce. Refresh it when a
  // shopper submits a page left open past the nonce lifetime.
  nocache_headers();
  wp_send_json_success(['nonce' => wp_create_nonce('sunstreaker_boundaries')]);
}

add_action('wp_ajax_sunstreaker_mockup_nonce', 'sunstreaker_handle_mockup_nonce');
add_action('wp_ajax_nopriv_sunstreaker_mockup_nonce', 'sunstreaker_handle_mockup_nonce');

function sunstreaker_handle_generated_mockup_asset_upload() {
  $nonce = isset($_POST['_ajax_nonce']) ? (string) wp_unslash($_POST['_ajax_nonce']) : '';
  if (!wp_verify_nonce($nonce, 'sunstreaker_boundaries')) {
    wp_send_json_error(['message' => 'Bad nonce'], 403);
  }
  ...
  if ($kind !== 'mockup' && $kind !== 'original_back') {
    wp_send_json_error(['message' => 'Invalid generated asset kind.'], 400);
  }

  $upload_error = isset($_FILES['image']['error']) ? (int) $_FILES['image']['error'] : UPLOAD_ERR_NO_FILE;
  if ($upload_error === UPLOAD_ERR_INI_SIZE || $upload_error === UPLOAD_ERR_FORM_SIZE) {
    wp_send_json_error([
      'code' => 'mockup_upload_too_large',
      'message' => 'The personalized preview exceeds the server upload limit.',
    ], 413);
  }
  if ($upload_error !== UPLOAD_ERR_OK || empty($_FILES['image']['tmp_name'])) {
    wp_send_json_error(['message' => 'No generated image received.'], 400);
  }

  add_filter('upload_dir', 'sunstreaker_generated_mockup_asset_upload_dir');
  require_once ABSPATH.'wp-admin/includes/file.php';
  $upload = wp_handle_upload($_FILES['image'], [
    'test_form' => false,
    'mimes' => [
  ...
  remove_filter('upload_dir', 'sunstreaker_generated_mockup_asset_upload_dir');
...
function sunstreaker_generated_mockup_asset_upload_dir(array $uploads): array {
  $subdir = '/sunstreaker-generated-assets/'.gmdate('Y/m');
...
add_action('wp_ajax_sunstreaker_upload_generated_mockup_asset', 'sunstreaker_handle_generated_mockup_asset_upload');
add_action('wp_ajax_nopriv_sunstreaker_upload_generated_mockup_asset', 'sunstreaker_handle_generated_mockup_asset_upload');
