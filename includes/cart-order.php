...
function sunstreaker_get_posted_generated_asset_url(string $field): string {
  $field = sanitize_key($field);
  if ($field === '' || !isset($_POST[$field])) return '';

  $url = esc_url_raw((string) wp_unslash($_POST[$field]));
  if ($url === '' || !filter_var($url, FILTER_VALIDATE_URL)) return '';

  if (function_exists('sunstreaker_local_path_from_url')) {
    $path = sunstreaker_local_path_from_url($url);
...
function sunstreaker_apply_posted_generated_assets(array $cart_item_data): array {
  if (empty($cart_item_data['sunstreaker']) || !is_array($cart_item_data['sunstreaker'])) {
    return $cart_item_data;
  }

  $mockup_png_url = sunstreaker_get_posted_generated_asset_url('sunstreaker_mockup_png_url');
  if ($mockup_png_url !== '') {
    $cart_item_data['sunstreaker']['mockup_png_url'] = $mockup_png_url;
    $cart_item_data['sunstreaker']['mockup_url'] = $mockup_png_url;
    $cart_item_data['sunstreaker']['mockup_source'] = 'browser';
  }

  $original_back_png_url = sunstreaker_get_posted_generated_asset_url('sunstreaker_original_art_back_png_url');
  if ($original_back_png_url !== '') {
    $cart_item_data['sunstreaker']['original_art_back_png_url'] = $original_back_png_url;
    $cart_item_data['sunstreaker']['original_art_back_source'] = 'browser';
  }
...
function sunstreaker_render_saved_cart_thumbnail(array $cart_item): string {
  $ss = !empty($cart_item['sunstreaker']) && is_array($cart_item['sunstreaker']) ? $cart_item['sunstreaker'] : [];
  $url = isset($ss['mockup_png_url']) && is_string($ss['mockup_png_url']) ? trim($ss['mockup_png_url']) : '';
  if ($url === '' || !filter_var($url, FILTER_VALIDATE_URL)) return '';
  $url = esc_url($url, ['http', 'https']);
  if ($url === '') return '';

  // Use the browser-generated preview so the cart preserves the customer's layout.
  return '<span class="sunstreaker-cart-thumb"><img src="'.$url.'" alt="'.esc_attr__('Customized product preview', 'sunstreaker').'" style="display:block;width:100%;height:auto;max-width:none;object-fit:contain;" /></span>';
}

function sunstreaker_render_cart_thumbnail(array $cart_item): string {
  $saved_thumbnail = sunstreaker_render_saved_cart_thumbnail($cart_item);
  if ($saved_thumbnail !== '') return $saved_thumbnail;
...
add_filter('woocommerce_add_cart_item_data', function($cart_item_data, $product_id, $variation_id){
  ...
  $cart_item_data = sunstreaker_apply_posted_generated_assets($cart_item_data);
...
add_action('woocommerce_checkout_create_order_line_item', function($item, $cart_item_key, $values, $order){
  ...
  if (!empty($ss['preview_image_height'])) {
    $item->add_meta_data('_sunstreaker_preview_image_height', absint($ss['preview_image_height']), true);
	  ...
	  if (!empty($ss['mockup_webp_url'])) {
	    $item->add_meta_data('_sunstreaker_mockup_webp_url', esc_url_raw((string) $ss['mockup_webp_url']), true);
	  }
	  if (!empty($ss['mockup_url'])) {
	    $mockup_url = esc_url_raw((string) $ss['mockup_url']);
	    $item->add_meta_data('Mockup', $mockup_url, true);
	  }
  if (!empty($ss['mockup_source'])) {
    $item->add_meta_data('_sunstreaker_mockup_source', wc_clean((string) $ss['mockup_source']), true);
  }
  if (!empty($ss['original_art_back_png_url'])) {
    $item->add_meta_data('_sunstreaker_original_art_back_browser_png_url', esc_url_raw((string) $ss['original_art_back_png_url']), true);
  }
  if (!empty($ss['original_art_back_source'])) {
    $item->add_meta_data('_sunstreaker_original_art_back_source', wc_clean((string) $ss['original_art_back_source']), true);
  }
...
add_filter('woocommerce_order_item_get_formatted_meta_data', function($formatted_meta, $item) {
  $hidden_lookup = array_fill_keys([
	    ...
	    'Original Art - Front',
	    'Original Art - Back',
	    'Original Art Front PNG',
	    'Original Art Front EPS',
	    'Original Art Back PNG',
	    'Original Art Back EPS',
	    'Original Art — Front',
	    'Original Art — Back',
  ...
  foreach ($formatted_meta as $meta_id => $meta) {
    ...
    if (is_object($meta)) {
      ...
      if (isset($meta->key)) {
        ...
        if ($display_key === '') $display_key = $raw_key;
      }
      if (isset($meta->value) && is_scalar($meta->value)) {
        $raw_value = (string) $meta->value;
      }
    }

    if (
      ($raw_key !== '' && strpos($raw_key, '_') === 0)
      || ($display_key !== '' && strpos($display_key, '_') === 0)
      || ($display_key !== '' && isset($hidden_lookup[$display_key]))
      || ($display_key !== '' && stripos($display_key, 'Original Art') === 0)
    ) {
      unset($formatted_meta[$meta_id]);
      continue;
    }

    if ($display_key === 'Mockup' && $raw_value !== '') {
      $mockup_url = esc_url($raw_value);
      if ($mockup_url !== '') {
        $meta->display_value = '<img src="'.$mockup_url.'" alt="'.esc_attr__('Mockup', 'sunstreaker').'" style="display:block;width:72px;height:72px;object-fit:cover;border:1px solid #dcdcde;border-radius:4px;background:#fff;" />';
      }
    }

    if ($display_key === 'Name/Number Add-on') {
      $name_number_addon_meta = [$meta_id => $meta];
      unset($formatted_meta[$meta_id]);
    }
  }

  if (!empty($name_number_addon_meta)) {
    $reordered = [];
    $inserted = false;
    foreach ($formatted_meta as $meta_id => $meta) {
      $reordered[$meta_id] = $meta;
      $display_key = is_object($meta) && isset($meta->display_key) ? (string) $meta->display_key : '';
      if ($display_key === 'Back Font') {
        foreach ($name_number_addon_meta as $addon_meta_id => $addon_meta) {
          $reordered[$addon_meta_id] = $addon_meta;
        }
        $inserted = true;
      }
    }
...
add_filter('woocommerce_hidden_order_itemmeta', function($hidden) {
  ...
  $sunstreaker_hidden = [
    '_sunstreaker_scrubs',
    '_sunstreaker_font_choice',
    '_sunstreaker_right_chest_font_choice',
    '_sunstreaker_logo_locations',
    '_sunstreaker_logo_ids',
    '_sunstreaker_logo_id',
    '_sunstreaker_logo_labels',
    '_sunstreaker_logo_urls',
    '_sunstreaker_logo_url',
    '_sunstreaker_ink_color',
    '_sunstreaker_preview_image_url',
    '_sunstreaker_preview_image_width',
    '_sunstreaker_preview_image_height',
    '_sunstreaker_mockup_svg_url',
    '_sunstreaker_mockup_png_url',
    '_sunstreaker_mockup_webp_url',
    '_sunstreaker_mockup_source',
    '_sunstreaker_front_art_url',
    '_sunstreaker_front_art_label',
    '_sunstreaker_front_transform',
    '_sunstreaker_back_art_url',
    '_sunstreaker_back_art_label',
    '_sunstreaker_back_transform',
    '_sunstreaker_size',
    '_sunstreaker_original_art_generated',
    '_sunstreaker_original_art_generated_at',
    '_sunstreaker_original_art_version',
    '_sunstreaker_original_art_error',
    '_sunstreaker_original_art_svg_url',
    '_sunstreaker_original_art_eps_url',
    '_sunstreaker_original_art_png_url',
    '_sunstreaker_original_art_size',
    '_sunstreaker_original_art_font',
    '_sunstreaker_original_art_width_in',
    '_sunstreaker_original_art_back_browser_png_url',
    '_sunstreaker_original_art_back_source',
    '_sunstreaker_front_original_art_generated',
    '_sunstreaker_front_original_art_generated_at',
    '_sunstreaker_front_original_art_error',
    '_sunstreaker_front_original_art_eps_url',
    '_sunstreaker_front_original_art_png_url',
    '_sunstreaker_mockup_generated',
    '_sunstreaker_mockup_generated_at',
    '_sunstreaker_mockup_error',
    'mockup_url',
    'Original Art Front',
    'Original Art Back',
    'Original Art - Front',
  ...
  return array_values(array_unique(array_merge($hidden, $sunstreaker_hidden)));
}, 20, 1);
